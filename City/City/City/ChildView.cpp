...
void CChildView::OnLButtonDblClk(UINT nFlags, CPoint point)
{
    ...
    tile->Accept(&visitor);
    if (visitor.IsRocketPad())
    {
        auto landing = visitor.PrevRocketTile();
        CTileRocketPad* rocketTile = visitor.FindRocket(&mCity);
        if (!rocketTile->GetRocket().get()->InFlight())
        {
            rocketTile->GetRocket().get()->SetLandingPad(landing);
            landing->SetRocket(rocketTile->GetRocket());
            OnPaint();
        }
    }
}
