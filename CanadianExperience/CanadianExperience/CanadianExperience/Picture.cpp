...
void CPicture::Save(const std::wstring &filename)
{
    ...
    // Loop through Machine Drawables
    for (auto machine : mMachines)
    {
        // This machine's Name
        auto name = machine->GetName();

        // Save Start frame
        root->SetAttribute(name + L"StartFrame", machine->GetStartTime());

        // Save Machine Number
        root->SetAttribute(name + L"Number", machine->GetMachineNumber());
    }
...
void CPicture::Load(const std::wstring &filename)
{
    ...
    try
    {
        ...
        for (auto machine : mMachines)
        {
            ...
            // Load Start frame
            int startFrame = root->GetAttributeIntValue(name + L"StartFrame", 0);
            machine->SetStartTime(startFrame);

            // Load Machine Number
            int number = root->GetAttributeIntValue(name + L"Number", 0);
            machine->SetMachineNumber(number);
        }
